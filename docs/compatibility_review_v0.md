...
4. **Backend names and explicit execution.** `native-cpu`, `native-cuda`, `native-mps`, `native-tn`, and `native-mpi` are the reviewed fixed explicit backend names. CUDA also has the reviewed canonical parameterized family `native-cuda:<device>` for a nonzero visible device ordinal; device zero canonicalizes to `native-cuda`. Hardware- or build-dependent backends must fail explicitly when unavailable.
